---
stt: 012
timestamp: 2026-08-16T15:22+07:00
model: claude-opus-5
scope: [backend, frontend, prompts, docs, README.md]
---
...
end to end, đảm bảo chạy local work tốt nhé (local backend and frontend) @/d:/VSCODE/22127472/SPECRESEARCH_LOOP/docs/SYSTEM_DESIGN_ANALYSIS.md

## Kết quả
Hiện thực hoá toàn bộ đồ án từ scaffold trống: backend NestJS (Prisma 7 + Neon, auth JWT cookie,
LLM gate DeepSeek, sources S2/OpenAlex/Crossref/DataCite, verifier 5 tầng, 5 judge song song,
vòng sửa + version + diff, export MD/PDF) và frontend Next.js 16 (wizard 5 bước, 3 tầng bố cục,
DecisionSheet, CardBoard, JudgePanel, DiffView) — chạy end-to-end thật với dữ liệu thật.
Thêm 15 file `prompts/*.md`, bộ eval 4 arm (`backend/eval/`), `docs/evaluation_report.md`, README.
Chạy thật phát hiện và sửa hai bug làm sai lệch metric: L0 chỉ hỏi Crossref nên DOI arXiv (DataCite)
bị gắn SOURCE_NOT_FOUND oan; OpenAlex `search=` khớp lỏng trả về paper lạc đề hoàn toàn.
