# Prompt — sinh Design System + Architecture cho SpecResearch Loop

> File này **không phải tài liệu dự án**. Nó là prompt để copy nguyên khối vào một phiên agent mới
> (khuyến nghị: bật plan mode). Output là 2 file `docs/DESIGN_SYSTEM.md` + `docs/ARCHITECTURE.md`
> để người dùng verify rồi dùng làm plan implement.
>
> Cách dùng: mở phiên mới → paste toàn bộ phần trong khối dưới → agent tự đọc docs rồi viết.
...
6. Layout & responsive: BẮT BUỘC responsive cả desktop lẫn mobile. Chốt số breakpoint tối thiểu,
   cách 3 cột biến đổi ở tablet và mobile, bảng nhiều cột đổi sang pattern gì, và vùng chạm.
   Không được nói "không làm mobile" — luật đó đã bị bỏ.
...
Verify theo checklist cuối prompt, rồi mới sang giai đoạn 0 của `docs/STACK.md` §9.
Nếu agent đề xuất công nghệ ngoài STACK.md — đó là dấu hiệu nó chưa đọc kỹ, bắt sửa trước khi dùng.
